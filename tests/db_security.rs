...
fn test_sql_injection_locale_sanitization() {
    ...
    conn.execute_batch("CREATE TABLE posts (id TEXT PRIMARY KEY, title__en TEXT, title__de TEXT)").unwrap();
    conn.execute("INSERT INTO posts (id, title__en, title__de) VALUES ('1', 'Hello', 'Hallo')", []).unwrap();
    ...
    assert!(result.is_ok(), "Query should succeed despite malicious locale (due to fallback/sanitization)");
...
fn test_sql_injection_via_union_in_locale() {
    let conn = Connection::open_in_memory().unwrap();
    conn.execute_batch("CREATE TABLE posts (id TEXT PRIMARY KEY, title__en TEXT, title__de TEXT)").unwrap();
    conn.execute("INSERT INTO posts (id, title__en, title__de) VALUES ('1', 'Post 1', 'Beitrag 1')", []).unwrap();
    
    // Create another table we want to steal data from
    conn.execute_batch("CREATE TABLE users (id TEXT PRIMARY KEY, password_hash TEXT)").unwrap();
    conn.execute("INSERT INTO users (id, password_hash) VALUES ('admin', 'SECRET_HASH')", []).unwrap();
