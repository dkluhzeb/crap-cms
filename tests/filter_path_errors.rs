...
use crap_cms::service::{
    CountDocumentsInput, DeleteManyOptions, FindDocumentsInput, OpDeadline, RunnerReadHooks,
    ServiceContext, ServiceError, UpdateManyOptions, count_documents, delete_many, find_documents,
    update_many,
};
use serde_json::json;
...
/// `posts` with a `title` and an `items` array whose rows hold a `name`.
fn setup() -> Setup {
