...
const crap = {
  ...
  drawer: {
    open(opts) {
      discoverSingleton('crap:drawer-request')?.open(opts);
    },
    close() {
      discoverSingleton('crap:drawer-request')?.close();
    },
  },

  deleteDialog: {
    open(opts) {
      discoverSingleton('crap:delete-dialog-request')?.open(opts);
    },
  },

  createPanel: {
    open(opts) {
      discoverSingleton('crap:create-panel-request')?.open(opts);
    },
    close() {
      discoverSingleton('crap:create-panel-request')?.close();
    },
  },
