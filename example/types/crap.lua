...
--- Only used when `[upload] storage = "custom"` in crap.toml.
--- @class crap.storage
crap.storage = {}

--- Storage handler functions for custom backends.
--- @class crap.StorageHandlers
--- @field put fun(key: string, data: string, content_type: string)  Store a file. `data` is raw binary (Lua string). `key` is e.g. `"media/abc123_photo.jpg"`.
--- @field get fun(key: string): string                              Retrieve a file. Returns raw binary data (Lua string).
--- @field delete fun(key: string)                                   Delete a file. No error if key doesn't exist.
--- @field url fun(key: string): string                              Return the public URL for a key (e.g., CDN URL).
--- @field exists? fun(key: string): boolean                         Check if a key exists (optional — falls back to get).

--- Register custom storage handlers for the `"custom"` backend.
--- Called in `init.lua`. All four core functions (`put`, `get`, `delete`, `url`)
--- are required. `exists` is optional.
---
--- Example:
--- ```lua
...
---   end,
---   get = function(key)
---     local resp = crap.http.request({ url = "https://storage.example.com/" .. key })
---     return resp.body
---   end,
---   delete = function(key)
---     crap.http.request({ method = "DELETE", url = "https://storage.example.com/" .. key })
---   end,
---   url = function(key)
...
--- })
--- ```
--- @param handlers crap.StorageHandlers  The storage handler functions.
function crap.storage.register(handlers) end
