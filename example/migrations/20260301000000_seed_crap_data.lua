...
-- `posts.content` stores rich text as a ProseMirror JSON document
-- (`admin.format = "json"`), so post bodies are written as documents, not
-- HTML. These builders cover the nodes the seed uses; the node and mark names
-- are the editor's (`heading`, `paragraph`, `text`, the `strong` mark).
local rt = {}

-- A text node, optionally with marks (e.g. `{ { type = "strong" } }`).
function rt.text(text, marks)
  return { type = "text", text = text, marks = marks }
end

-- Bold text.
function rt.bold(text)
  return rt.text(text, { { type = "strong" } })
end

-- Inline content from plain strings and text nodes.
local function inline(parts)
  local content = {}
  for _, part in ipairs(parts) do
    content[#content + 1] = type(part) == "string" and rt.text(part) or part
  end
  return content
end
...
-- A level-2 heading.
function rt.h2(text)
  return { type = "heading", attrs = { level = 2 }, content = inline({ text }) }
end

-- A paragraph of strings and text nodes.
function rt.p(...)
  return { type = "paragraph", content = inline({ ... }) }
end

-- A document of block nodes.
function rt.doc(...)
...
function M.up()
  ...
  create_post({
    ...
    related_content = { "projects/" .. proj_apex_ecom.id },
    content = rt.doc(
      rt.h2("Performance is a Feature"),
      rt.p("Every 100ms of latency costs 1% of sales. For Apex Retail, we obsessed over every millisecond.")
    ),
  ...
  create_post({
    ...
    categories = { cat_design.id },
    content = rt.doc(
      rt.h2("Lighting is Everything"),
      rt.p("Good product photography starts with lighting. Here are the setups we use most often at the studio.")
    ),
  ...
  create_post({
    ...
    tags = { tag_webgl.id },
    content = rt.doc(
      rt.h2("Your First Scene"),
      rt.p("Three.js makes WebGL accessible. In this guide, we'll build an interactive 3D scene from scratch.")
    ),
  ...
  create_post({
    ...
    tags = { tag_ux.id },
    content = rt.doc(
      rt.h2("Five Days, Fully Remote"),
      rt.p("We've run over 20 remote design sprints. Here's the process, tools, and tips we've refined along the way.")
    ),
