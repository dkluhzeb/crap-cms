...
- **A `join` field inside an array or blocks row is refused as a filter path.** `items.posts` or `content.meta.posts` naming a `join` field was accepted and compared a value that is never stored (an empty column or an absent key), while the same field at the top level was already refused. It is now a validation error naming the path, at any depth, on every surface; the in-memory evaluator (access constraints, live events) matches nothing for it.
- **An upload's claimed content type must be one concrete type, and `mime_type` stores the detected one.** A multipart file part claiming a pattern (`image/*`, `*/*`) or a malformed type was used as a pattern itself — `image/*` matched whatever image the bytes turned out to be and passed an `image/*` allowlist — and was stored verbatim as the document's `mime_type`. Such a claim is now rejected; send the file's real type. `mime_type` now holds the type sniffed from the bytes when they are recognisable (the claimed type otherwise).
- **An upload's `focal_x` / `focal_y` must lie within `0.0`–`1.0`.** The focal point was documented as a fraction of the image but its columns had no bounds, so any surface stored a point outside the image. A value outside the range is now a validation error on every write surface.
- **A job's `timeout` must be at least one second.** `crap.jobs.define({ timeout = 0 })` was accepted, and every run of the job then timed out the moment it started. It now fails the load naming the job; `crap-cms make job --timeout 0` is refused too (it used to drop the `0` and scaffold the default).
- **A join field's `on` must reference the collection owning the join.** `on` was never checked: a name that is not a field of the target, a field referencing another collection, a has-many or polymorphic relationship, or a field inside a group booted and the join silently listed nothing (or, for a group field, listed documents on a single-document read but nothing in a list). The load now fails naming the field unless `on` is a has-one, single-target relationship or upload field at the top level of the target collection (layout wrappers are transparent) whose target is the owning collection. A join in a global is rejected the same way — nothing can reference a global.
- **A null value reaches Lua as `nil` in every context.** The field-hook, validation (`validate` / `required_when`), access, live-filter, route, job, auth-strategy and MFA contexts, the render-hook `info`, and the tables `crap.*` calls return (`validate`, `delete_many`, `find` pagination, `list_versions`, `crap.http.request`, `crap.schema.*`) represented a JSON null — and an absent optional value — as a light-userdata sentinel, which is truthy in Lua: `ctx.data.x == nil` was false and `if ctx.user then` took the branch for a null field or an anonymous request. They now hold `nil`, like the collection hook context and every document table already did, and a null key is simply absent from its table. Code that treated such a value as present — a truthiness test, `type(v) == "userdata"` — now sees it as unset. A null **array element** is the new `crap.null` sentinel instead (so the array keeps its length; `crap.json.decode` does the same, where it used to leave a `nil` hole that truncated the array), and `crap.null` is how Lua writes an explicit null back: a hook, job, route or Lua CRUD call that must clear a field or keep a present-null key uses `{ field = crap.null }` — a `nil`-valued key is absent. An access rule that reads a NULL `ctx.user` field and then returns a filter table is now denied (see Security).
- **Globals refuse `hooks.before_delete` / `hooks.after_delete`.** A global is never deleted, so those hooks never ran; defining one now fails the load, naming the key, like the delete-side access keys already did. `make hook --global` no longer offers them.
- **An invalid `versions` setting fails the load.** A `max_versions` that is negative, fractional, text or above 4294967295 was read as `0` — unlimited history — without a word; a `versions` value that is neither a boolean nor a table was ignored. Both are load errors now, naming the key.
- **MCP write tools follow the write shape of an upload collection.** The `create`, `create_many`, `update`, `update_many` and `validate` input schemas advertised the server-derived upload columns (`filename` — required on create — `url`, `mime_type`, `filesize`, `width`, `height`, the per-size columns), which every write strips. They are no longer advertised, and a value sent for one is rejected as an unknown field, like any other key the schema does not list.
- **Regenerate your Lua types: the hook and query classes follow the runtime.**
  - `crap.data.<Slug>` / `crap.global_data.<slug>` (a hook's `ctx.data`) declare every field optional: an update's before-hooks see only the fields the request sends, so no key — required or not — can be promised. The create payload (`crap.input.<Slug>`) keeps `required`.
  - `ctx.operation` names every value the runtime passes: `crap.HookContext` adds `undelete`, `unpublish` and `restore`; `crap.hook.<Slug>` adds `undelete`; `crap.read_hook.<Slug>` adds `undelete`, `unpublish` and `restore` (the live events it shapes), a global's `crap.read_hook.global_<slug>` adds `unpublish` and `restore`. The typed hook contexts also declare `edited_by`.
  - `crap.field_hook.<Slug>` / `crap.field_hook.global_<slug>` declare `id`, `locale`, `document` and `options`, and type `data` as `crap.data.<Slug>|table<string, any>` — a nested field's `data` is its group object or row; the document is `document`.
  - `crap.where.<Slug>` no longer offers `_status` / `_deleted_at` (a user filter may not name them), and the `order_by` values of `crap.query.<Slug>` drop has-many list columns (never sortable) and add the documented `"_rank"`.
  - `crap.hook_fn` (and `crap.hooks.register` / `remove`) return `crap.HookContext|false|nil`, so a `before_broadcast` hook that suppresses with `return false` type-checks; `crap.hooks.register("before_render", function(ctx, info) … end)` is typed through the new `crap.render_hook_fn` and `crap.template.render_info` classes; `crap.hooks.list` returns an array of hook functions (its type read as a function returning an array).
- **An admin form's has-many value is a JSON array, and one plain value is one element.** A `<select multiple>` (or any field submitted more than once) was comma-joined and split again, so an option value containing a comma (`10,5 cm`) came back as two values. A repeated field now reaches the server as a JSON array of its values, and a single plain value of a `has_many` select, radio, text or number field is that one value — it is no longer split on commas. The multipart upload API (`/api/upload/{slug}`) reads form fields the same way: send a has-many field as a JSON array or repeat the field once per value.

- **Filters on has-many fields match element by element.** A `text`, `number`, `select` or `radio` field with `has_many = true` compared its whole stored JSON text, so `{ tags = "news" }` never matched `["news","tech"]` on any surface (API, MCP, Lua, admin, access rules). Every operator now reads the list's elements: `equals`, `like`, `contains`, `in`, the ordered comparisons and `exists` match when **some** element does, `not_equals`, `not_in` and `not_exists` when **no** element does; a number list compares numerically. A has-many relationship or upload filtered by `.id` follows the same rule — `not_equals`, `not_in` and `not_exists` used to mean "some related id differs" (a post tagged `a` and `b` matched `tags.id not_equals a`) and now mean "no related id is". `exists` on a has-many list no longer matches an empty list. A has-many relationship or upload inside an array or blocks row (`items.related`, `content.links`) used to be compared as its whole stored text; it now reads its ids the same way, a polymorphic entry by the id after its `collection/`. An empty list and an unset list hold no elements.
- **Has-many values stored before their field held a list are stored as lists at startup.** Every stored value of a `has_many` text/number/select/radio field, and the id list of a has-many relationship or upload inside an array or blocks row, is now always NULL or a list. The schema sync rewrites any other value once, whenever a collection's or global's has-many fields change (and once on upgrade): a single value becomes a one-element list (`'news'` → `["news"]`, `5` → `[5]`), a JSON array spelled as text keeps its elements, a list's elements take the field's type, and blank text becomes NULL. Other text reads by where it is stored: in a document's own column (top-level, per locale, or a group's prefixed column) it is one value — `'Hello, world'` becomes `["Hello, world"]` — while inside an array or blocks row, where earlier admin forms stored a list as comma-separated text, `"a,b"` becomes `["a","b"]`, for a value list and a relationship list alike. A version or draft snapshot kept from before the switch reads the same way. A value that holds nothing of its field's type — text in a `number` list, a polymorphic entry that isn't `collection/id` — stops startup with an error naming the collection, column and document instead of being dropped. A has-many relationship or upload inside a row is also written as its id list from now on, whatever spelling the write carried.
- **Sorting by a has-many list field is rejected.** `order_by` on a `has_many` text/number/select/radio field ordered the stored JSON text; it is now a validation error on every surface, the admin list offers no sort header for it, and an `admin.default_sort` naming one fails startup.
- **`_status` can no longer be mixed with other fields in an admin OR filter.** `where[or][G][N][_status][equals]=draft` next to another field in the same OR group used to be lifted out of the group and applied to the whole list, so "(draft) OR (title = B)" ran as "draft AND title = B". The list now answers 400 for that combination. An OR group of only `_status` rows (a status union) and a single-bucket group still work.
- **`admin.default_sort` on a `hidden` field fails startup**, naming the collection. A hidden field is never sortable, so every list load was refused.
- **Operator-localized labels follow the viewer's UI locale.** A per-locale `admin.label`, `labels.singular` / `labels.plural`, select option label, block label, placeholder or description resolved to its alphabetically first key for everyone (a `{ en = …, de = … }` label showed German to English viewers). The admin now resolves it for the viewer's admin UI locale, then `locale.default_locale`, then the first key; the gRPC/REST schema endpoints, MCP and `crap.schema` outside an admin request resolve against `default_locale`.
...
- **A `_block_type` filter outside a block row is refused.** `content.meta._block_type` or `variants._block_type` — a group or an array row, which has no block type — was accepted and read as an absent value, so `not_exists` matched every such row. The path is now a validation error; `_block_type` follows the block row it names (`content._block_type`, `content.nested._block_type`).
- **Relationship, upload and join targets must be defined collections.** A `relationship` or `upload` field — at any depth, and every target of a polymorphic list — or a `join` field naming a collection that is not defined used to load, then failed startup's reference-count recompute ("relation does not exist") and every write that adjusts reference counts, and the generated client types referenced a type that does not exist. The load now fails naming the collection or global, the field path and the target. An `upload` field must also target an upload collection (`upload = true`); the docs always required it, but nothing checked.
...
- **Generated input types describe what a write accepts; read types what a read returns.** Regenerate your client and Lua types after upgrading.
  - **TypeScript `…Data` (input) types** carry every relationship and upload as its id (`string` / `string[]`, a polymorphic one as its `"collection/id"` string) instead of `string | TDocument` — every write surface rejects a populated document. A `required` single relationship or upload is now required in the input (it was optional, which let a create that the server rejects type-check). A virtual `join` field and an upload collection's server-derived columns (`filename`, `mime_type`, `filesize`, `width`, `height`, `url`, the per-size columns) are no longer declared — the server strips them from every write that is not a file upload, so `filename` was a required key a typed client could never usefully send. An auth collection's input gains an optional `password`, which gRPC and MCP accept beside the fields.
  - **Read types no longer declare `hidden = true` fields** (every read strips them, nested ones included) in any language or in the Lua `crap.doc.*` / `crap.global_doc.*` classes; the Rust proto decoder no longer reads them.
  - **A collection read type declares the `collection` tag** a populated copy carries (TypeScript `collection?: "posts"`, Python `Optional[Literal["posts"]]`, Go `Collection *string`, Lua `collection? "posts"`), so a polymorphic union narrows on it. Rust keeps consuming it as the polymorphic enums' serde tag.
  - **`_status` is `"draft" | "published"`** in TypeScript and Python (it was `string`), matching the Lua types; Rust and Go keep a string.
  - **Go: a system key keeps its member name.** A field named like a system key (`deleted_at`, `draft_status`) used to take `DeletedAt` / `DraftStatus` and push the system key to `DeletedAt_2`; the system keys now claim their names first, so it is the user field that gets the suffix. The optional `collection` tag is the exception: it never renames a field — beside a field whose member is also `Collection` (a field named `Collection` or `collection_`) the tag member becomes `Collection_2`, still decoding the `collection` key.
  - **Lua:** `create`, `create_many` and `validate` take the new `crap.input.<Slug>` class, `update` takes `crap.partial.<Slug>` and `update_many` the new `crap.partial_many.<Slug>`, all built from the write shape — `crap.input` and `crap.partial` add `password` on an auth collection, `crap.partial_many` never has one (`update_many` refuses it), and the nested `crap.array_row.*` / `crap.group.*` classes no longer declare a virtual `join`; `crap.data.<Slug>` stays the stored shape a hook's `ctx.data` holds. `crap.where.<Slug>` and the `order_by` values of `crap.query.<Slug>` no longer offer a `hidden` field's columns, which every find, count and search refuses. `crap.doc.<Slug>` types a relationship or upload as `string|crap.doc.<Target>` (reads populate at the default depth) and a JSON-format rich text field as `table` (the other classes as `string|table`: a write takes the document or its JSON text); its nested rows and groups are the new read-shape classes `crap.doc_row.*` / `crap.doc_group.*`.
...
- **Hidden and read-denied fields can no longer be filtered or sorted on.**
  ...
  the value the read strip removes. This holds at any depth of the path: a
  group sub-field (`seo.secret`, `seo__secret`), an array-row or block
  sub-field (`items.secret`, `content.body`) and a field nested inside a row
  are judged by their own rule and by the rules of every group, array or
  blocks field on the way; a block path is rejected when the field is denied
  in any block type holding it. The `where` filter of the bulk writes
  `update_many` and `delete_many` is judged the same way on every surface,
  a queued bulk job included (refused when it is queued, not only when it
  runs) — their `modified` / `deleted` / `skipped`
  counts and the `bulk_max_documents` "matched N documents" error counted
  rows by the hidden value; an `override_access` context is exempt, as from
  every access rule. Hidden and read-gated fields are also
...
- **No stored value may contain a NUL character, at any depth.** The NUL check covered only top-level `text`, `textarea` and `email` values. A NUL anywhere else — a group sub-field, an array or blocks row, a has-many list, a `code`, `json` or `richtext` value — was stored: on Postgres a raw one failed the write with an internal error, and one inside a row's JSON (or a draft snapshot) was stored as the `\u0000` escape, which Postgres' `::jsonb` cast rejects — so a single writer made every row-path filter on the collection, and the drafted-file lookup of the upload serve route, fail for every caller. Every string of a write is now checked at any depth, object keys and the decoded content of JSON-bearing values included, on both backends: validation reports `validation.nul_character` on the offending field (`items[0][label]`), the final data a write stores is checked again after `before_change` hooks and on writes that skip validation (`hooks = false` bulk writes), and `crap-cms import` refuses a document holding one.
- **A `hidden` field is unfilterable in every spelling of its path.** The hidden-field check compared the query path with the flat form its denial carries, so a hidden sub-field of an array inside a group (`seo__links.url`) could still be filtered or sorted on as `seo.links.url` — a value oracle on a field no response returns. A group's part of a path now compares the same spelled `.` or `__`.
- **An access constraint the in-memory evaluator judges no longer passes on an operand SQL refuses.** A row constraint whose value does not fit its field — a word or `inf` against a Number, a non-boolean against a Checkbox — is refused by SQL (the read returns nothing, or a block type reading the path that way matches none of its rows), but the in-memory evaluator (live events, population gating, draft reads) compared it as text, so `not_equals` / `not_in` matched every document. It now matches nothing for such an operand, under every operator.
- **A served SVG keeps its sandboxing Content-Security-Policy.** The admin security-headers layer wraps the `/uploads` route and replaced every response's CSP with the admin page policy — so the `sandbox; default-src 'none'` policy the upload route sets on SVG never reached the browser, and a script in an SVG opened directly ran with the admin origin's authority. A response that sets its own policy now keeps it.
- **The upload extension check reads the stored name.** It judged the raw filename, while the file is stored and served under its sanitized name — `evil.htm l` has no known extension raw but is stored as `evil.html`, and was served as HTML whatever its content. The check now judges the sanitized name.
- **An upload write the collection's access rule refuses no longer processes the file.** The admin create/update forms stored the file — decode, every resize, the synchronous format conversions — before any access check, and only the write refused the row afterwards, so a caller with no write access could make the server do all of that on every request. The collection's `create` / `update` rule is now consulted once the file has been validated and before it is stored, on every surface — on the request's fields together with the columns the file already determines (`filename`, `mime_type`, `filesize`, and `width` / `height` for an image), so a rule gating on `ctx.data.mime_type` judges the file it lets in. A file that fails validation is reported as such (`400`) before the rule is consulted.
- **The public upload fast path honours `before_read` hooks.** A collection with no read rule, drafts or soft delete served its files without a lookup and cached them `public, immutable` — even when a `before_read` hook (the collection's own or a registered global one) refuses the read on every other surface. Such a collection now goes through the per-document gate.
- **A join no longer lists documents whose `on` field the viewer cannot read.** A join lists the target documents whose `on` field holds this document; the read strip removed a `hidden` or read-denied `on` value from each listed document, but the document's presence in the list still revealed it. A populated join (any surface, at any nesting depth) and the admin join field — its items and its count — now leave out every document whose `on` value the strip removed, judged per document, so a data-dependent `access.read` rule keeps exactly the documents it allows.
- **The delete dialog's back-references no longer list references held in a field the viewer cannot read.** The report filtered referrers by whether the viewer could read the referring *document*, not the referring *field*, so it revealed which documents hold the target in a `hidden` field or a field whose `access.read` rule (or a group, array or blocks field on its path) denies the viewer. Such groups are now dropped — judged by the same rule, in the default locale, that decides whether the viewer may filter on the field — and only the non-quantified "also referenced by documents you don't have access to" note remains. The report's `field_name` now carries the field's full dotted path (`meta.hero` for a field inside a group, where it used to be the bare `hero`; `meta.items.image` for an array inside a group).
- **A custom-strategy credential can no longer be exchanged for a session token.** `POST /admin/api/session-refresh` reissued a signed `crap_session` JWT from the claims of any authenticated request — including one a custom strategy authenticated, whose internal claims were indistinguishable from a session's. A strategy credential (an API key, an SSO header) thereby became a long-lived bearer token usable on every surface, surviving the strategy credential's revocation. Strategy claims are now marked as such (`token_use = "strategy"`) and the token provider refuses to sign them, and the refresh endpoint extends only a session the `crap_session` cookie established: a request authenticated by a strategy or a bearer token gets `401`.
- **A strategy's returned `_verified` / `_locked` flags can only restrict.** On the per-request path a `_verified = true` on the table a strategy returned verified an account the stored row says is unverified, while the login path ignored a returned `_locked`. Every path — per-request, login, and the external auth callbacks — now admits a strategy's user through one check: the stored row decides, and the returned table may only lock (`_locked = true`) or unverify (`_verified = false`), never unlock or verify.
- **The external auth callback no longer mints a session for a trashed user, and builds it from the stored user.** `/admin/auth/callback/…` checked that the returned id existed with a lookup that counts trashed rows, and ran the `admin.access` gate and built the session's email from the table the callback hook returned. It now admits the user like a custom strategy's (a stored, non-trashed, unlocked and — where required — verified row) and gates and mints from the stored document.
- **The external auth callback enforces the collection's MFA.** `/admin/auth/callback/…` minted the session straight after the callback hook named a user, so on a collection with an `mfa` mode an OAuth / OIDC login skipped the second factor a password or strategy login must complete. A callback now passes the same MFA gate (including `mfa_when`): on an MFA collection it redirects to `/admin/mfa` with a pending-MFA cookie, the user completes the collection's mode (TOTP, email code or custom delivery), and only then is the session issued. A collection whose identity provider already enforces 2FA can exempt that callback by name with the new `password_login` option `mfa_exempt_callbacks = { "<name>" }` (a startup error without an `mfa` mode).
- **A trashed user can no longer use a password-reset or email-verification link.** The token lookups had no soft-delete filter, so a link issued before the account was moved to the trash still reset its password or verified it.
- **`crap-cms user reset-totp` ends the user's sessions.** The reset cleared the TOTP enrollment but left every session established with the old second factor alive. Like every other credential change it now bumps the session version — in the same statement as the reset — and tears down the user's open live streams (reaching `serve` over Redis live updates).

- **A custom auth strategy's user is the stored document.** The request user of a strategy-authenticated request was the table the strategy returned, where a NULL field's key is dropped, so the NULL-field access guard had nothing to track and `{ tenant_id = ctx.user.tenant_id, archived = false }` widened to every tenant's rows for such a user; a document found through a Lua read also lacked its hidden fields. The returned table now only names the user by its `id`: the request, and a login a strategy completes, carries the stored document read exactly as for a bearer-token or session-cookie request. A returned `id` naming no stored user of the collection, or a trashed one, is refused, and fields the strategy adds to the returned table are no longer carried into `ctx.user`.
- **An access constraint built from a NULL user field is denied.** A NULL field reaches Lua as `nil`, and a `nil`-valued key drops out of a table constructor, so `{ tenant_id = ctx.user.tenant_id, archived = false }` for a user whose `tenant_id` is NULL became `{ archived = false }` — every tenant's rows. Only the single-key form (an empty table) was denied. Now any collection, global or field access rule that reads a NULL `ctx.user` field (at any group depth) and then returns a filter table is denied, with a warning naming the collection, the operation and the field(s). A `true` / `false` / `nil` verdict is unaffected. A rule that probes an unrelated NULL field before returning its constraint (`if ctx.user.role == "admin" then return true end; return { owner = ctx.user.id }` with a NULL `role`) is denied too — give the field a default, or read it with `rawget(ctx.user, "role")`, which is not tracked.
- **The example project's ownership rules trusted the incoming patch.** `access/author_or_editor.lua` (posts) and `access/team_or_admin.lua` (projects) granted an update when `ctx.data.author` / `ctx.data.team` named the caller — but an update's `ctx.data` is the request's own patch, so an author could take over any post by sending `author = <own id>`, and a team member was refused any update that did not resend `team`. The post rule now returns a filter table enforced on the stored row (`{ author = … }`) and refuses a patch that hands the post to someone else; the project rule loads the stored project (`find_by_id` at `depth = 0` with `override_access = true`) and allows only a caller listed in its `team` — a has-many relationship cannot be an access constraint, which must reference a flat own column. Projects started from the example should apply the same change; the access-control docs now warn explicitly that an update's `ctx.data` is the caller's patch.
- **gRPC `Validate` passed an auth collection's `password` to `before_validate` hooks.** The dry-run kept the plaintext password in the field data its hooks saw; `Create` / `Update` and the MCP and Lua validate calls split it off. `Validate` now does too.
- **Rich text parses stored HTML into an inert document.** The editor loaded a stored value through `innerHTML` on an element of the live admin page, where an `<img onerror>` in the stored HTML runs its handler (blocked only while the default Content-Security-Policy is on). The value is now parsed with `DOMParser`, whose document never loads or runs anything.

- **Row-constrained live-event subscribers received events for documents their read rule hides — and never learned of deletes they could see.** A `metadata`-mode collection's events and every delete carry no document, and the in-memory constraint check read a field missing from the payload as satisfied by a negative operator — a read rule such as `{ tags = { not_in = { "secret" } } }` or `{ reviewed = { not_exists = true } }` let the SSE stream and gRPC `Subscribe` deliver the id, operation and editor of every such event, secret documents included. Every event now carries a snapshot of the stored row it concerns — the row as stored, hidden and read-denied fields, `id` and timestamps included; for a hard delete the row as read just before removal, for a soft delete the row as trashed — and a subscriber's row constraint is judged against that snapshot, in both modes, on both streams, for single and bulk writes on every surface. So a constrained subscriber now receives a `metadata`-mode event or a delete exactly when the row satisfies its constraint (still metadata only), and a `full`-mode event is judged by its stored row rather than by a payload a `before_broadcast` hook may have reshaped. The snapshot is used only for this check and is never delivered to any subscriber or passed to the `live` filter or `before_broadcast` hooks. A field the snapshot does not carry matches no operator; a field it holds as `null` matches `not_exists`, as in SQL. An event without a snapshot — from a node that predates it during a rolling upgrade on the Redis transport, or one whose snapshot exceeds the Redis transport's 512 KiB snapshot cap — reaches only unconstrained subscribers. On the Redis transport the event channel therefore carries each written document's stored row, hidden and read-denied fields included, on every write in both live modes: run that Redis as trusted, access-controlled infrastructure (see the multi-server deployment guide). Unconstrained subscribers are unaffected. A permanent delete of a document that was in the trash — a purge of the trash, or a forced permanent delete of a trashed document — was gated by the document's status view, so a subscriber without `trash` access learned of a document it could no longer see; it is now gated by the `trash` view, the view the document was last in, and its snapshot carries the trash timestamp a `trash` rule may name.
...
- **Read-denied fields are no longer a query oracle.** See the breaking
  ...
  read-gated fields are refused instead of answered — and so is a bulk
  `update_many` / `delete_many` filter on one, whose counts answered the
  same question.
...
- **A block row whose type declares no field of the filtered name reads it as absent.** With one shared definition such a row read the name as NULL, but once the declaring block types defined it differently (a number in one, text in another) the row matched nothing — so adding a second, differently-typed definition flipped `not_exists` and `not_in = {}` results. Such a row now reads NULL in both cases, at the top level and in nested block rows, on both backends and in the in-memory evaluator.
- **Cursor pagination of an all-locales read sorted by a localized field pages correctly.** The document holds the field as a `{ en, de }` map, and the cursor recorded that map's JSON text while the SQL orders by the default locale's column — the next page compared the column against JSON. The cursor now records the default locale's value, for a top-level and a group field alike.
- **A dotted group sort (`seo.title`) pages correctly on every read.** It was rewritten to its column (`seo__title`) only by the `find` operation, so a read reaching the list service another way (the admin search, the upload visibility checks) encoded its cursor for a name no document holds, with a NULL sort value. Every list read now normalizes the sort.
- **`not_in` an empty list matches a NULL value in the in-memory evaluator too.** SQL renders `not_in = {}` always-true, NULL included, while live-event and population gating left a NULL value (or a key a row lacks) unmatched.
- **Back-references judge a referring field's `read` rule in the default locale.** The delete dialog's back-reference report passed no locale to the referring field's `access.read` rule, so a locale-aware rule judged it differently than a filter on the same field. Every access decision in the report — the referring field's rule, the owner's `read` rule and its row filter — now runs in the default locale, as filters and bulk writes judge them.
- **A checkbox inside a row's JSON can be filtered on Postgres.** A filter on a checkbox inside a block row, or inside a group / nested row of an array row (`content.done = true`), compared the stored `'true'` text with an integer operand and failed with a parameter error; the stored `true`/`false` now compares as `1`/`0` on both backends, under every operator. The in-memory evaluator's ordered comparisons (`>`, `<`, `>=`, `<=`) on a checkbox now read `true`/`false` as `1`/`0` too, as SQL does, instead of never matching.
- **Block types that name a field alike but define it differently are filtered per block type.** The first block type declaring a name decided how every row was read: a number in one block type and text in another cast the text on Postgres (an error), a has-many list in one and a single value in another expanded the single value as a list ("malformed JSON"), and a group in one type behind a scalar of the same name in another could not be reached. Each block row is now read with its own block type's field — on both backends and in the in-memory evaluator; a path valid for at least one block type is accepted and matches only rows of those types, and an operand that fits no block type's field is a validation error.
- **An array, blocks or has-many field inside a group can be filtered.** `seo.links.url`, `seo__links.url` or `seo.tags.id` was rewritten to one flat column name (`seo__links__url`) and rejected; such a path now reaches the field's own join table (`{collection}_{group}__{field}`), with the field's localized rows read in the reading locale as hydration reads them, and errors name the path as written.
- **Cursor pagination sorted by a group's sub-field pages correctly.** The cursor read the sort value by its flat column name (`seo__title`) from the returned document, which holds the group nested, so every cursor carried a NULL sort value and the next page restarted among the NULLs.
- **`/api/upload` no longer judges the access rule without the request.** The REST upload routes consulted the collection's `create` / `update` rule before reading the request, with no data at all, as well as through the write — so a rule reading `ctx.data` (`ctx.data.mime_type`, an owner field) refused uploads it allows. The rule is now judged once, by the write (the delete route's duplicate check is gone too; the delete itself still judges its rule).
- **An editor can view a file uploaded with a draft save.** A replacement file stored by a draft save is named only by the draft's version snapshot, and the upload serve gate matched files against document rows only — so the edit form's preview of the drafted file answered 404 for the very editor who uploaded it. The file now serves to exactly the viewers whose draft view shows that draft; a viewer without draft access still gets 404.
- **Restoring a version brings back its queued image variants.** A format converted on the background queue is written to the live row by its job, never to the snapshot taken when the file was stored, so restoring that version left the variant's column empty for good (and the restore deleted the variant the replaced row still named). A restore now names again every queued variant whose bytes the document still holds, re-queues the ones it no longer holds, and a restore that swaps the file cancels the previous file's still-queued conversions.
- **Format negotiation only looks for variants that can exist.** Every request for an image — originals included — probed storage for a `.avif` / `.webp` sibling, though variants are only ever generated beside the resized sizes and only for the formats a collection configures; on a remote backend that was two wasted round-trips per original. Originals, non-images and collections without `format_options` are now served without a variant lookup and without `Vary: Accept`.
- **A Lua job's `timeout` stops the handler, and a timed-out run never overlaps its own retry.** The timer only abandoned the blocking task: the handler kept running — and committing — while the run was failed and re-queued, so the retry ran next to it outside every concurrency cap and `skip_if_running`, and both runs' writes landed. A Lua handler now stops itself at its deadline: the VM's instruction hook (armed for the handler's run even with no instruction limit configured — other Lua runs without a limit stay hook-free) and every database, `crap.http` and `crap.email` call raise `job exceeded its timeout`, and the transaction scope re-checks the deadline before `COMMIT`, so the operation in flight rolls back instead of committing late (writes committed before the deadline stay). The scheduler no longer releases a run while it still executes: the row stays `running` — heartbeat fresh, slot held — until the run has returned, and a run still executing past its timeout (plus five minutes for a self-limiting run) is logged instead of re-queued. Email and image-convert runs, which are bounded by their own I/O, are likewise never retried while still running.
- **Accounts created through Lua CRUD in a job, a custom route, a migration or `on_init` get their verification email.** Those surfaces carried no email context, and their per-write verification queue had nowhere to go after the commit, so the pending verification was dropped without a word: the account existed with no token and no email queued. Lua CRUD on every surface now mints the token and queues the email on the write's own transaction, exactly like a service write; a verification that cannot be issued is logged instead of vanishing.
- **`skip_if_running` also skips while a previous run is still queued.** It counted only `running` runs, so a run waiting to be claimed — or failed and waiting out its retry backoff — did not block the next cron fire, and a slow job stacked one more run per fire.
- **A job's heartbeat is written only for the attempt the worker is running.** The heartbeat update had no compare-and-set, unlike every other job-row write, so a worker still holding a reclaimed run kept the heartbeat of a later attempt fresh — hiding that attempt's worker's death from stale recovery.
- **A finished job run frees its slot immediately.** The scheduler only claimed new runs on its poll tick, so with the default per-job `concurrency = 1` a queue of short jobs drained at one run per `poll_interval`. Every finished run now wakes the scheduler, which claims again at once; the poll tick remains the fallback.
- **The "SMTP host empty — using log provider" notice (and the plaintext-SMTP warning) is logged once per process**, not once per Lua VM — two dozen identical lines per boot.
- **Password reset and email verification work on an auth collection with localized fields.** The reset and verification token lookups selected the bare field names, which are not columns on a localized collection (`bio__en` is, `bio` is not), so the reset page, the reset submit (admin and gRPC) and email verification all failed there. They now read the default-locale columns, like the login lookup.

- **`admin.default_sort = "_ref_count"` fails startup instead of every list load.** The startup check accepted the reference-count column as a sort, while every find refuses to order by it (its value counts references from collections the reader may not see), so the collection's list view failed on each request. The check now refuses it, naming the collection.

- **Startup no longer drops an index that only resembles a managed one.** The schema sync removes the stale `idx_{slug}_…` indexes of a collection table, but matched that prefix with SQL `LIKE`, where `_` stands for any character (and SQLite ignores case), so an index you created yourself on the table — say `idx0posts_x`, or `IDX_POSTS_X` on SQLite — was dropped at every start. Only names that really start with the managed prefix are touched now, on both backends.

- **Saving a document no longer clears references the editor cannot see.** A relationship or upload whose target the viewer may not read — or that is trashed — rendered with no selection, so the form submitted an empty value and saving silently removed the stored reference; a read-only field lost it just the same. Every stored reference now stays selected: one that cannot be shown appears as a generic "Unavailable item" (no title is revealed) and is submitted back unchanged unless the editor removes it. Polymorphic relationships inside a group, array or blocks row were affected the same way (has-many was never resolved, has-one was looked up in the wrong collection); each `collection/id` ref now resolves in its own collection. Template overrides can tell these items apart by `unavailable: true` on the `selected_items` entry.
- **The password toggle on the login and password-reset pages is labelled.** The auth layout shipped no translations to the page's JavaScript, so the toggle announced the raw key `password_show`. Both layouts now render the translations island from the new `partials/i18n-island.hbs` partial (an overridden `layout/auth.hbs` needs `{{> partials/i18n-island}}` in its `<head>`). The toggle is now a proper toggle button: its label stays "Show password" and `aria-pressed` alone reports whether the value is shown, instead of also swapping the label to "Hide password".
- **CSRF refusals are translated.** The "security check failed" and "request too large to check" answers (plain-text body and error toast) were English only; they now use the admin default locale.
- **Filters reach every row path, and a block row's id.** A filter into an array row could only continue into a group sub-field: a nested array or nested blocks inside an array row (`variants.sizes.label`, `variants.parts._block_type`) was refused, while the same shapes inside a block row were followed at any depth. An array row's nested JSON is now walked exactly like a block row's. `content.id` — a block row's own id, which every read returns and every write round-trips — was refused while `variants.id` was accepted; both filter by the row's id now. The in-memory evaluator (live-event gating, population, draft reads) reads the same paths the same way.
- **Array and blocks row tables are indexed on `parent_id`.** Loading a document's rows, every filter into an array or blocks field (the `EXISTS` / `NOT EXISTS` row checks) and the cascade delete from the parent looked rows up by `parent_id`, which no index covered, so each scanned the whole row table. The schema sync now creates `idx__rows_{table}` on `(parent_id)` — `idx__lrows_{table}` on `(parent_id, _locale)` for a localized field — on new and existing tables on both backends, and switches the form when a field gains or loses localization. A name that would pass Postgres's 63-byte identifier limit keeps its prefix and the start of the table name and ends with `_` and 16 hex digits of a hash of the table name, so a long row table name never refuses the boot. On an existing database the first start builds these indexes inside the schema-sync transaction; on Postgres, writes to a row table from other instances wait until it commits.
- **Date filters with a bare day cover the whole day.** The admin filter builder (and any API caller) sends `YYYY-MM-DD` for a date, but a Date field bound it as the day's noon, so `equals` only matched a value stored at exactly 12:00 and `greater_than` took in the afternoon; `created_at` / `updated_at` compared it as plain text against full timestamps, so "is D" matched nothing, "on or before D" left out D and "after D" took it in. A bare-day operand on a date (in a column, a group, an array or blocks row, a has-many list) and on `created_at` / `updated_at` now covers the whole UTC day `[D 00:00, D+1 00:00)` for `equals`, `not_equals`, the ordered comparisons, `in` and `not_in`; an operand with a time keeps its exact comparison. The in-memory evaluator (live events, population gating, draft reads) reads dates the same way, and now also normalizes the operand of an ordered date comparison as SQL does (`> 2026-01-15T09:00` no longer matched a value stored at 09:00 there).
- **`crap.collections.validate` / `crap.globals.validate` honour the hook depth cap.** The dry-run runs `before_validate` hooks but skipped the `[hooks] max_depth` check every other Lua CRUD call makes, so a hook that validated again recursed until the stack ran out.
- **Postgres boots no longer log a page of NOTICE lines.** Every `CREATE … IF NOT EXISTS` the schema sync runs answered with "relation … already exists, skipping", logged at INFO. Sessions now set `client_min_messages = 'warning'`; warnings and errors still arrive.
- **The example project:** `hooks/set_published_at.lua` overwrote `published_at` with the current time on every non-draft update that did not resend it (an update's `ctx.data` is only the patch) — it now stamps a document that has no date yet, reading the stored one; the seed migration's `down` could never succeed (soft-deleted posts and pages kept their references, and the last admin was protected) — it now removes the whole seed and `up` applies it again; `hooks/validate_budget.lua` reported the offending value as the limit (and failed on a fractional budget); comments in `plugins/rating.lua` and `hooks/prevent_last_admin.lua` described the wrong template key and delete-hook data; all example Lua is formatted with the project's `stylua.toml`.
- **`make job`'s template example counted the result table instead of its `documents`.**
- **Hook docs:** the hook-context page lists every `ctx.operation` per hook, states that an update's before-hooks see only the request's fields, and corrects the system-field (`id`, timestamps, `soft_delete`; `user` / `ui_locale` are context keys) and `draft` tables; `before_broadcast` is documented to fire for every live event (`undelete`, `unpublish`, `restore` included) on the request's thread; the globals page no longer splits its configuration table.
- **A rolled-back data migration or `on_init` hook no longer deletes upload files.** Lua CRUD in `crap-cms migrate up|down` and in `[hooks] on_init` ran on a bare transaction: hard-deleting an upload document removed its files at once, so a migration that then failed rolled the row back with its files gone. Both now run in the same transaction scope as every other Lua write: files are removed, the populate cache is cleared and live events are published only after the commit, and a failure leaves every file in place. A delete made inside a transaction that has no post-commit cleanup queue now keeps the files and logs them as orphaned instead of deleting them before the commit.
- **Removing or retargeting a relationship field recounts the references.** The reference-count backfill only re-ran when its computation version or the configured locales changed, so a relationship or upload field that was removed, renamed, pointed at another collection, turned into a list or localized kept its old references counted, and the target stayed undeletable. The backfill now also re-runs when the schema's relationship and upload fields change at any depth (inside groups, arrays and blocks, polymorphic targets included) or a collection holding references is removed. Adding or removing a collection or global without any relationship or upload field no longer recounts anything, and a newly added one is recorded without a recount while its table is empty (one that already holds rows — re-added after removal — recounts once). Existing databases recount once on the next startup.
- **`crap-cms db cleanup --confirm` keeps reference counts right.** Deleting junction rows of a dropped locale, dropping orphan columns and dropping orphan tables left the references they held counted, and the steps ran outside a transaction. The whole cleanup now runs in one transaction that recomputes every reference count before it commits, and its "Dropped:" / "Deleted:" lines are printed only once that commit succeeded — a cleanup that fails and rolls back no longer reports changes it never made.
- **`crap-cms user lock`, `user unverify` and `user change-password` end the user's live streams.** They wrote the account state directly, so the session-version bump (for `change-password`) happened but the user's open live-update streams kept running, although the docs promised a lock tears them down. They now go through the same service ops as the admin and gRPC, on the configured live transport: with Redis the streams on `serve` close.
- **`crap-cms import` clears the cache and ends overwritten accounts' streams.** After the commit, the import clears the configured populate cache and tears down the live streams of every existing account it overwrote.
- **`crap-cms user create` writes like every other surface.** The user was inserted with a bare row write: a has-many or array `-f` value never reached its join table, no version snapshot was taken, only the required-field prompt was validated, and no verification email was queued. It now creates through the service write (still without lifecycle hooks). Array, blocks and group values are given as JSON, list values as a JSON array (see Changed).
- **A relationship or upload inside an array or blocks row writes to its own row after the rows change.** Duplicating, removing, moving or nesting rows never renumbered the picker's `field-name`, and the picker rebuilt its hidden input from the name it read at first render — a pick in a duplicated row, or in any row after a removed or moved one, was submitted under another row's index (joined into that row's value, or a phantom row). Row renumbering now rewrites `field-name`, and the picker follows it.
- **The "View" link of a polymorphic has-one relationship opens the picked document.** It pointed at `/admin/collections/{first collection}/{collection}/{id}` — a 404.
- **The admin filter builder offers only `is` for the draft/published status.** It offered `is not`, which the list refuses for `_status`.
- **An admin upload over the size limit keeps the form and says why.** The refused body answered a plain redirect, which htmx followed into a blank form, losing every edit without a message. It now answers `413` with an error toast naming the limit (`422` with a toast for any other unreadable form), and the file input refuses an oversized pick before the form is sent. The upload API answers `413` too (it said `400`). On the routes that carry a file — an upload collection's admin create/update and `/api/upload` — the request body limit now follows that collection's `max_file_size`: a collection allowing larger files than `[upload] max_file_size` had those uploads refused by the body limit. Every other route (the login form, the JSON endpoints, MCP) keeps the global limit.
- **The unsaved-changes guard sees rich text and focal-point edits, and survives a refused save.** A rich text toolbar command or paste and a focal-point move fired no event, so leaving the page lost them without a prompt; both now fire `crap:change`. The guard cleared its flag as soon as a save was sent, so a save refused with a validation or access error left the page "clean"; it now clears only when the server accepts the save, and a validation re-render starts out marked unsaved.
- **The focal point can be set with the keyboard.** The picker is focusable and the arrow keys move the point (Shift for a finer step).
- **A refused search in a relationship picker says so.** A `403` or `500` from the search endpoint showed "No results"; the dropdown (and the browse drawer) now shows that the search failed.
- **CSRF refusals and a failed admin UI language switch show a toast.** A submit refused by the CSRF check (`403`, or `413` for a body too large to check) changed nothing visible under htmx; the language picker ignored a refused request.
- **An inline-create error is toasted once.** The create panel toasted a response's `X-Crap-Toast` itself and the page toasted it again.
- **Toasts are announced to screen readers** (the toast host is a polite live region; an error toast is an alert), and the admin's remaining hard-coded English labels — the rich text toolbar, the tag and chip remove buttons, the password toggle, the code editor's language picker, the filter builder's controls and the live-update banner's close button — are translated (English and German).

- **Filters on localized arrays, blocks and has-many relationships honour locale fallback.** With `fallback = true`, a read in a locale a document has no rows for shows the default locale's rows, but a filter on `tags.id`, `items.label` or `content.body` only looked at the requested locale's rows: the listed document was missed by `equals` / `exists` and wrongly matched by `not_equals` / `not_in` / `not_exists`. A filter now matches exactly the rows the read shows, on SQLite and Postgres. A `locale = "all"` read, which shows the default locale's rows, now filters on those too instead of on rows of any locale.
- **A bad filter field or path is invalid-argument on every operation.** Only `Find` pre-checked filter fields: `Count`, `UpdateMany` and `DeleteMany` with an unknown field answered gRPC `INTERNAL` (which clients retry on) and MCP "Internal error", and a bad dot-notation path (`content.nope`, `title.sub`, `tags.name`) did so on `Find` too. Every filter field and path is now a validation error naming it — `INVALID_ARGUMENT` over gRPC, the message over MCP and Lua — as is an invalid `order_by`. An array sub-field the rows do not have (`items.nope`) used to reach SQL as a column and fail there; it is now rejected up front, as is a path ending on a group or nested array inside an array row, and a nested group path inside an array row (`items.address.geo.lat`) is checked segment by segment.
- **`full`-mode live events carry the default locale's values.** A create or update written with a non-default `locale` published its localized fields in that locale, while a subscriber — who picks no locale — reads the default locale's, and a delete's event is judged in it. The event's document is now always the stored row read in the default locale, draft saves included.
- **The example's reading-time field always said "1 min read".** Its `after_read` hook read its own empty `reading_time` value instead of the post's `content`, and treated it as HTML although `content` is a JSON rich text document (a read returns it as a table, which made the hook raise). It now counts the words of `ctx.data.content` in either form.
- **The generated Lua types check clean in lua-language-server.** `types/hooks.lua` declared a leading-digit field as `---@field 2fa? string` (read by LuaLS as the integer `2` and a name `fa`), and a field named `public`, `protected`, `private` or `package` lost its name to LuaLS's field-scope syntax; such keys — and Lua reserved words — are now declared `---@field ["2fa"]? string`, which types `data["2fa"]`. A collection or global whose slug starts with a digit or is a reserved word produced `function crap.collections.2fa.hook(fn) end`, a syntax error; it is now reached and typed as `crap.collections["2fa"]`. The per-collection and per-global typing factories (`hook`, `read_hook`, `field_hook`, `condition`, `access`, `auth_strategy`, `row_label`) raised an `inject-field` warning for every collection and global; they are now declared on the accessor class, so they no longer warn and stay typed. In `types/crap.lua`, the `crap.storage.register` and `crap.cache.register` handler types listed `get: fun(key: string): string?, …`, which LuaLS read as `get` returning the rest of the table; the function types are now parenthesized.
- **`crap-cms typegen lua` rejects a schema whose Lua class names collide.** Slugs and field paths are `PascalCase`d into class names, so collections `a1` and `a_1` (or a `posts` group `seo_meta` and a `posts_seo` group `meta`) declared the same class and LuaLS merged them; generation now fails with an error naming the class, and nothing is written. Under `admin.dev_mode` the startup regeneration logs the error as a warning.
- **Top-level `_status` rows in an admin list URL are ANDed like every other row.** `where[_status][equals]=draft&where[_status][equals]=published` listed drafts and published documents alike; it now lists none, since no document is both (a `_status` row outside an OR group also narrows an OR group of `_status` rows instead of widening it). An OR group of `_status` rows is still a union. Contradicting rows show one filter pill each instead of "is any of".
- **The admin filter builder no longer builds a `_status` row into an OR group with other fields.** The server refused that shape with a 400. A row can now join the row above it with OR only when both filter `_status` or neither does; otherwise its OR choice is disabled, an OR already picked falls back to AND, and a hint explains why — also when a row's field is changed, rows are added or removed, or a hand-edited URL is reopened. `_status` rows can still be OR'd with each other. A crafted URL with that shape still gets a 400, now with its message in the viewer's UI language.
- **A filter on an array or blocks sub-field inside a layout row finds it.** A sub-field inside a `row`, `collapsible` or `tabs` of an array was looked up among the array's direct sub-fields only, so its filter lost the field's type (a number compared as text); inside a block it could not be named at all, the layout field's name ending up in the JSON path although a block row stores the field under its own key. Both are now named directly (`items.width`, `content.caption`).
- **A has-many relationship or upload inside an array or blocks row keeps its selection in the admin.** The admin form submits such a field as comma-separated ids, which were stored as that text; the edit form reads an id list, so on reopening the row showed no selection, and saving it again cleared the references. The ids are now stored as a list on every write, and the schema sync converts the lists stored before (see Breaking).
- **A has-many value kept from before its field held a list reads as that list.** A draft or version snapshot saved while a field was still single-valued read its `"news"` as an empty list once the field turned `has_many`, so the edit form showed nothing and saving dropped the value; it now reads as `["news"]`, the list a write of it stores.
- **A has-many list written as a single number keeps it.** A `has_many` field given one number or boolean instead of a list (an import of a hand-edited export, say) stored an empty list; it now stores a one-element list, as a single text value always did.
- **Two admin filter rows on the same field are ANDed, as the filter builder labels them.** `where[title][equals]=a&where[title][equals]=b` was merged into `title IN (a, b)` — documents matching either value — although both rows read AND; on a has-many list that answered "tagged a or b" for "tagged a and b". Rows now combine exactly as written; "any of" is an OR row (`where[or][G][N][…]`). Removing one of two such rows' pills keeps the other.
- **Filtering a has-many upload by `.id` works.** The filter validation accepted `gallery.id` but the query builder only routed relationships, so every such filter failed with "does not support sub-field filtering".
- **Hooks run by an admin request resolve labels in the viewer's UI locale.** A hook (or `crap.schema` inside it) run by the edit page, a create, update, delete, restore or empty-trash resolved a per-locale label against `locale.default_locale`, while the same hook on the list page followed the viewer's UI locale. Every admin operation now carries the viewer's label locale onto its blocking thread.
- **A `full`-mode live event carried only what the user who made the change could read.** The event's document was stripped by the editor's field-read access before it was published, then again per subscriber, so a subscriber allowed a field the editor may not read never received it. Each subscriber's payload is now stripped from the document as stored — by that subscriber's own field-read access, then of hidden fields, then through its `after_read` hooks — so it receives exactly what its own read returns. Hidden fields and fields a subscriber may not read are still never delivered.
- **The `live` filter and `before_broadcast` hooks see the stored document in every mode.** In `metadata` mode their `ctx.data` was empty, so a filter such as `return ctx.data.status == "published"` suppressed every event; in `full` mode it was the document stripped for the user who made the change. Both now receive the document as stored (read-shaped, hidden and read-denied fields included), as `after_change` does; a `metadata`-mode event still delivers none of it, and a `full`-mode payload is stripped per subscriber afterwards.
- **Purging the trash published no live events.** The scheduled retention purge, "Empty trash" in the admin and `crap-cms trash purge` / `trash empty` removed documents without a delete event, so a subscriber of the trash view never learned they were gone, while a single permanent delete did announce itself. Every purge now publishes each purged document's delete event after it commits, gated by the `trash` view, and clears the populate cache as a permanent delete does; the CLI publishes on the configured transport (with Redis, to `serve`'s subscribers). The admin collapses a burst of live events for one collection (or global) and operation — a purge, a bulk write with events — into a single toast carrying the count instead of one toast per document; a collection and a global sharing a slug keep separate toasts.
- **The scheduled retention purge works in bounded batches.** It purged all expired trash in one transaction, holding the write lock — and every purged row's delete event — until the whole backlog was done. It now purges at most 500 expired documents per transaction, deleting that batch's files and publishing its events after each commit, and continues until everything expired has been examined. A document that could not be purged failed its purge inconsistently: a row whose upload files could not be read was skipped, while one whose delete event could not be read failed the entire purge on every run (and on Postgres even the skip failed the run, since a failed statement aborts the transaction). Any document that fails to purge is now logged with its collection and id, left in the trash and retried by the next run, and the rest of the purge proceeds without it; once 8 documents of one collection fail in a run, that collection waits for the next run.
- **`crap-cms trash restore` restores through the service layer.** It cleared the trash timestamp directly: the collection's `before_change` / `after_change` hooks didn't run for the undelete, the cache kept reads made while the document was trashed, and no undelete event was published, so a subscriber (with Redis, on `serve`) never saw the document come back. It now restores through the same undelete as the admin UI, gRPC, MCP and the Lua API — hooks run with `ctx.operation = "undelete"`, and a `before_change` hook that errors leaves the document in the trash. `crap-cms user delete` now also clears the cache (a shared Redis cache is `serve`'s own).
- **Undeleting a live document ran its `before_change` hooks.** The undelete read its target with the trash included but never checked the row was in it, so undeleting a document that wasn't trashed ran `before_change` — and whatever the hook did outside the transaction — before failing with "not found". It now fails before any hook runs.
- **Access rules on `id` or a timestamp hid populated relationships and drafts.** A relationship target embedded by population, and a draft read from its version snapshot, were judged against the document's field map alone, which holds neither `id` nor `created_at` / `updated_at`, so a view rule such as `{ id = { equals = user.id } }` hid a target or a draft the same read returns from the database. Both are now judged on the document's whole row, like the live-event snapshot.

- **The MCP `create_*` schema no longer requires `password` on an auth collection.** The service accepts a create without one (an external auth method may own the credential), as gRPC and the Lua API do; a client that follows the schema can now create such a user.

- **Admin list view:**
  - Hidden fields and fields the viewer may not read are no longer offered as columns, sort headers or filters (clicking one answered "You don't have permission to view this collection"). A sort or filter the viewer asks for on such a field anyway is refused naming the field, and an `admin.default_sort` the viewer cannot read is dropped for that viewer instead of refusing the list. A has-many relationship is no longer offered as a filter the `where[]` parser then rejected.
  - The search form keeps the trash view, the filters, the sort and the page size; clearing the search keeps them too (it used to reset to the plain list).
  - `per_page` is carried in page, cursor and sort links, so page 2 no longer comes back at the default size overlapping page 1.
  - Removing a filter pill resets the page position, and removing the last one links to the list instead of re-requesting the filtered URL. `In` and `_status` filters get pills; OR groups count toward the filter badge.
  - "Empty trash" shows how many documents it will delete — the whole trash, not the filtered page — and stays available while a search narrows the trash view to nothing. The count used to render empty.
  - Filter pill operators are translated; the title column header shows the title field's label instead of its name; a search or filter with no results says "No results" instead of "No posts yet".
- **User-settings collision.** A collection named `ui_locale` shared its column preferences with the admin UI-locale preference; per-collection preferences now live under a `collections` key. Settings saved before are still read and move over on the collection's next column save.
- **Password-policy errors are translated.** The field error named a translation key (`validation.password_policy`) that did not exist, so the admin showed the raw key under the password field. Each violation now has its own key with its limit as a parameter (`validation.password_min_length`, `validation.password_max_bytes`, `validation.password_uppercase`, …, `validation.password_empty`). The admin create and edit forms no longer check the policy themselves (they toasted the English message); the violation is the inline `password` error in the viewer's UI locale, and the reset-password page renders it in the page's locale too. The unused `error_password_min_length` translation key (which claimed a 6-character minimum) is gone.
- **Admin translations:** the sidebar "Fields" heading had no translation key; the login, forgot-password and reset-password pages re-rendered after an error with an English page title; a malformed `translations/<locale>.json` override was skipped without a word — it is now reported in the log with the file and the reason.
- **Example project:** the seed migration wrote HTML into the JSON rich text field `posts.content` and a `{ collection, value }` table into the polymorphic `related_content`, so `migrate up` on a fresh example failed. Post bodies are now ProseMirror documents and the relationship uses the `"projects/<id>"` form. The `test_nesting` collection had no access rules and was unreachable under the default `access.default_deny`; it is now admin-only.
...
- **A `full`-mode live event with an oversized payload is published as a
  metadata event** instead of pushing an unbounded message into Redis. The
  512 KiB cap is measured on the document data alone; the event's gating
  snapshot of the stored row is bounded separately at 512 KiB, so a
  document under the cap always keeps its data however large its snapshot.
...
- **`order_by` accepts a group's sub-field in its dotted form.** `order_by = "-seo.title"` sorts like `-seo__title`, the way filters already accepted both spellings.
- **`crap.null` — an explicit JSON null in Lua.** Lua `nil` cannot be stored in a table, so a hook, job, route or Lua CRUD call had no way to write a null. `crap.null` converts to `null` on every Lua→JSON path (hook return data, CRUD write data, route responses, job results, validation data, `crap.json.encode`): `crap.collections.update("posts", id, { subtitle = crap.null })` clears the field. Reading data, a null array element is `crap.null` so the array keeps its length. The sentinel is truthy; compare with `v == crap.null`. Typed in `types/crap.lua`.
- **Redis over TLS with `rediss://` URLs.** `[cache] redis_url` and `[auth] rate_limit_redis_url` accept `rediss://user:password@host:port/db`, which connects the cache, the rate limiter and the live-update event and invalidation transports over TLS. The server certificate is verified against the operating system's trust store; the `#insecure` URL fragment is not supported and fails the connection. Previously a `rediss://` URL failed at startup, so Redis traffic — including the live-update channel, which carries stored rows — could only be encrypted by a tunnel. A `rediss://` and a `redis://` URL for the same host, port and database count as the same Redis for the namespace-overlap checks.

- **Typed `after_read` hooks in Lua:** `crap.collections.<slug>.read_hook(fn)` / `crap.globals.<slug>.read_hook(fn)` and the `crap.read_hook_fn.<Slug>` alias type `ctx` as the new `crap.read_hook.<Slug>` class, whose `ctx.data` is the document as the read returns it (`crap.doc.<Slug>` — hidden fields stripped, upload sizes folded, references populated). `crap-cms make hook` scaffolds an `after_read` hook with it. The typed hook contexts now also declare `ctx.id` and `ctx.options`, which the runtime always set.

- **List page context:** `search_params`, `clear_search_url`, `is_filtered`, `trash_total` and `title_label` on `collections/items` (see the template context reference).
...
- **Queued bulk operations: `queue = true` on `CreateMany` / `UpdateMany` /
  ...
  and every authentication method can queue — bearer token, session cookie, or
  a custom strategy (whose user is always a stored row, carrying its stored
  session version). Queued runs are readable only by the identity that queued them and never
...
- **`crap-cms user create` takes JSON for structured and list `-f` values.** An array, blocks or group field takes a JSON value (`-f links='[{"url":"https://example.com"}]'`); a value that isn't JSON is an error naming the field. A list field takes a JSON array (`-f roles='["admin","editor"]'`); a has-many relationship or upload still takes comma-separated ids. Every other value is text, converted and validated by the write. A field inside a `row`, `collapsible` or `tabs` wrapper is typed and prompted for like a top-level one — a required field inside a wrapper used to be skipped by the prompts, so `user create` (and the `init` wizard's first user) failed validation with no way to supply it interactively. A group holding a required sub-field is prompted as one JSON object, and prompts for structured and list fields name the JSON format they take.
- **CLI writes that reach `serve` fail early without Redis.** `migrate up|down|fresh`, `import`, `user create`, `user lock|unlock|verify|unverify` and `user change-password` now build the same infrastructure as `user delete`: a configured Redis that can't be reached fails the command before it writes — for `migrate fresh`, before any table is dropped (it used to reach Redis only after emptying the database).
- **`on_init` hooks run after the server's infrastructure is assembled.** They still run before the server accepts requests and still abort startup on failure.
...
- `query::find_by_reset_token` / `query::find_by_verification_token` take `(conn, def, token, locale_ctx)` — the slug comes from the definition and the locale context selects a localized collection's columns. The admin session refresh reads the evaluator's `ResolvedMethod`, which the auth middleware now inserts into the request extensions beside the claims. `service::auth::admit_strategy_user` is the one admission check behind custom strategies (per-request and login) and the external auth callbacks; `service::auth::reset_totp` is the service op behind `user reset-totp`.

- **System-level Lua runs in its own scoped transaction.** `HookRunner::run_migration` takes a `MigrationCall`, the pool, the Lua CRUD infra and a bookkeeping closure that runs in the migration's transaction, and owns that transaction; `run_system_hooks_with_conn` became `run_system_hooks_in_tx`. `commands::user_lock` / `user_unlock` became `user_account_action`, backed by `service::auth::apply_account_action`; `service::auth::set_password` is the operator password change. The stale-locale row delete moved from the read queries to the write queries.
- The in-memory filter evaluator reads a path into array and blocks rows the way the SQL subquery does — some row, at any nesting, whose value satisfies the filter, a list in it element by element — and a has-many relationship's `rel.id` path from the ids the document carries (plain, polymorphic `collection/id` or populated), instead of reading either as an absent field. Access rules still reject dotted paths, so no live-event or population enforcement changes.
- Split by concern, no behavior change: the Lua class renderer into shared
  blocks plus collection, global and query classes; the TypeScript printer
  into the construct printer and the field-type mapping; the admin list read
  into request parsing and the blocking fetch. Work an admin request moves onto
  a blocking thread goes through one helper that carries the viewer's label
  locale. The CLI trash purge's bookkeeping (skipped documents, files and
  delete events owed after the commit) moved into its own module.

- The typegen golden snapshots cover every field type, both wire shapes, an
  auth, an upload-with-sizes and a join collection, and a drafts-enabled
  localized global, and now also pin the `typegen proto` output and the
  per-config Lua types. A structural test parses the Rust client types and the
  proto decoder with `syn` and checks that every decoder builds exactly the
  fields of its client struct. Regenerate with
