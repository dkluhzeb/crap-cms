...
| `admin` | Whether the collection/global is visible/usable in the **admin UI** (nav entry + its admin routes). A boolean rule — a returned filter table is logged as an error and treated as a **denial** (there is no row scope at this gate). **Permissive default** (visible when unset) — only ever *further* restricts admin-UI access beyond `read`. Valid on collections **and** globals. | **allow** (visible) |
