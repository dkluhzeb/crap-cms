...
| `required_when` | string | — | Lua predicate ref (`"module.fn"`) for a **conditional** requirement. The predicate receives the [validate context](#custom-validation) and returns truthy when the field must be present — e.g. require `shipping_address` only when `ctx.data.product_type == "physical"`. For a field inside an array/blocks row, `ctx.data` is the row (also for a field in a group within the row) and `ctx.document` the full document. Applies in addition to a static `required = true`, produces the same `validation.required` error, and is skipped on draft saves. |
...
| `data` | table | The **nearest scope**: the full document for a top-level field, or the current row for a validator on a field inside an array/blocks row. A group never narrows it — a field in a group sees the document at the top level and the row inside a row |
