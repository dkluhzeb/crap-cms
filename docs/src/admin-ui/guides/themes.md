...
### 2. Drop it into themes/

The simplest path: drop `themes-<name>.css` into your config dir's
`static/styles/themes/` folder, then add a single line to a styles
override:
...
└── static/
    └── styles/
        ├── main.css                       # overlays the entry — adds the @import
        └── themes/
            └── themes-acme.css            # your theme tokens
```

In your overlay `main.css`, copy the upstream chain and append your
theme:
...
@import url("/static/styles/themes/themes-acme.css");
```
...
See [CSS Variables](../reference/css-variables.md) for the full token catalogue,
