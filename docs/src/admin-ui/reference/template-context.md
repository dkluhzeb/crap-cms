...
- **`datetime_local_value`** (Option&lt;string&gt;) _(optional)_ — Set when `picker_appearance == "dayAndTime"` — the `YYYY-MM-DDTHH:MM` slice for the `<input type="datetime-local">`, or `YYYY-MM-DDTHH:MM:SS` when the stored value carries seconds.
- **`step`** (Option&lt;string&gt;) _(optional)_ — The input's `step` — `"1"` when the shown value carries seconds, so the browser shows and re-submits them instead of dropping them on save.
- **`min_date`** (Option&lt;string&gt;) _(optional)_
- **`max_date`** (Option&lt;string&gt;) _(optional)_
