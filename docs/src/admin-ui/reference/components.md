...
| `partials/array-row-header.hbs` | Drag handle + toggle + title slot + error badge + 4 action buttons; consumed by `<crap-array-field>`. Takes `expanded`, `has_errors`, `readonly` — `readonly` drops the drag handle and the action buttons, keeping the collapse toggle |
