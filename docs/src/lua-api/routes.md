...
| `access` | no | `omitted`/nil = **public**; `false` = registered but **disabled** (404); a hook ref = **gated** (evaluated before the handler; `true` → allowed, `false`/`nil` → 403, a filter table → hook error, any other value → 403 with a warning). `access = true` is **rejected** — it would silently mean "public", a footgun for a caller expecting "require auth"; omit `access` for a public route or pass a hook ref to gate it. |
| `rate_limit` | no | `{ max, window }` (window in seconds), per-IP. |
| `csrf` | no | `true` enforces the admin double-submit CSRF token on mutating methods (POST/PUT/PATCH/DELETE). The token is accepted either as the `X-CSRF-Token` header (what htmx requests carry) or as the `_csrf` field of a `application/x-www-form-urlencoded` body (what a plain `<form>` submit carries) — the admin layout emits both, so either shape from an admin page works. Default `false` (custom routes are API-style). `csrf = true` on a route with **only** safe methods (GET/HEAD/OPTIONS) is **rejected at load** — CSRF is never enforced on safe methods, so it would be a no-op; a safe-method handler must not mutate state. |
...
a hook ref; it receives the same `ctx` and returns `true` to allow or `false`
/ `nil` to deny. Returning a filter table (what a collection access rule
returns) is a configuration error — the request fails with a hook error, since
a route has no rows for a filter to narrow — and any other type denies with a
warning:
