...
# http_max_body_bytes = "1MB" # Max POST /mcp body size (and stdio request line)
...
| `request_timeout` | integer/string | `60` (`"60s"`) | How long an admin HTTP request may take to arrive and be answered: reading its body (a slow or stalled body included) and producing the response head. Exceeded → `408 Request Timeout`, and a `408` means the request changed nothing: a write still running at the deadline — a save, or an account change such as a login's strategy provisioning its user, a logout, an MFA or reset-token check, held up by a slow hook or a lock — is rolled back (its commit is refused; a save's statements are also cut off at the deadline), and a request whose change had already committed is answered by its handler instead of `408`. Applies to every admin route — custom routes and MCP HTTP included — except the routes that take a file into an upload collection, which follow `upload_timeout`. A streamed response body (SSE, a file download) is never cut. `0` disables it. Accepts seconds or human-readable (`"30s"`, `"5m"`). |
...
| `job_tools` | `false` \| `"read"` \| `"all"` | `false` | Background-job MCP tools. `false` exposes none; `"read"` adds `list_jobs` / `get_job_run` / `list_job_runs` (introspection, failure triage, and polling for queued bulk ops); `"all"` also exposes `trigger_job`. A job's own `access` hook still applies — it runs with `ctx.user = nil` and gates both listing and triggering — but the tiers exist because MCP otherwise runs with override access. The `queue` argument on the bulk tools appears only from `"read"` up. `true` is rejected as ambiguous. |
...
| `http_max_body_bytes` | integer/string | `1048576` (1 MiB) | Maximum request-body size for `POST /mcp`, and the maximum request line on the stdio transport. Accepts integer bytes or a filesize string (`"16MB"`). Raise for large bulk payloads or `write_config_file` assets. |
