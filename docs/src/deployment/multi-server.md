...
- Schema sync (`migrate up`) only needs to run once — any server that starts first handles it. Nodes starting at the same moment take turns: on Postgres, schema sync holds a database lock. Running it against a live cluster is safe for the other nodes' prepared statements: a statement whose plan the schema change invalidated is re-prepared and retried once, transparently, outside a transaction; a transaction caught mid-flight fails once and the next one succeeds. A start that changes nothing finishes its sync in moments. A start that does change the schema holds the tables it changes locked until its sync commits — other nodes' reads and writes of those tables wait (and fail once `[database] statement_timeout` passes). The longest such step is rebuilding a collection's search index, which runs only when the index's shape changed (its `list_searchable_fields`, their locale columns, the rich text nodes they index) or a first-start conversion rewrote the collection's stored text; an unchanged index is kept, since every write keeps it current. Schedule starts that change searchable fields like any other schema change.
