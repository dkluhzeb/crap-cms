...
- **System columns** — auth collection tables get hidden `_`-prefixed columns that are never exposed in API responses, hooks, or admin forms: `_password_hash`, `_reset_token`, `_reset_token_exp`, `_locked`, `_settings`, `_session_version` on every auth collection; `_verified`, `_verification_token`, `_verification_token_exp` when `verify_email = true`; `_mfa_code`, `_mfa_code_exp` when an MFA mode is enabled; `_totp_secret`, `_totp_confirmed`, `_totp_last_step` when `mfa = "totp"`.
...
- **Auth methods** — each auth collection declares a `methods` list (`password_login`, `bearer`, `session_cookie`, and any number of Lua `strategy` entries scoped by `surfaces` and `activates_on`). A single evaluator applies a fixed precedence — bearer → cookie → always-active strategies → header-activated strategies — see [Auth Methods](auth-methods.md). OAuth2, Cloudflare Access, AD, and API-key auth are implemented as strategies.
- **MFA** — built-in multi-factor authentication. Enable `mfa = "email"` on an auth collection to send a 6-digit code after password verification, `mfa = "custom"` with an `mfa_deliver` hook to send the code via your own channel (SMS, push, …), or `mfa = "totp"` for authenticator-app codes (RFC 6238 — no delivery; the secret is provisioned on the first challenge and confirmed by the first successful code). Completed on the admin MFA page or via the gRPC `VerifyMfa` RPC. The optional `mfa_when` hook scopes when the second factor applies (per surface or per user field). See [Multi-Factor Authentication](mfa.md) for the full mode comparison, the TOTP enrollment lifecycle, and the security model.
