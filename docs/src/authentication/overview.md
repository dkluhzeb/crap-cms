...
- **Email MFA** — built-in multi-factor authentication. Enable `mfa = "email"` on an auth collection to send a 6-digit code after password verification; completed on the admin MFA page or via the gRPC `VerifyMfa` RPC. The optional `mfa_when` hook scopes when the second factor applies (per surface or per user field).
