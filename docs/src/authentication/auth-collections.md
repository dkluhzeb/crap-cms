...
The CLI publishes the stream teardown on the configured live transport. With Redis live updates it reaches a running `serve` right away. With the default in-process transport, `serve` doesn't hear it: the revoked session is still rejected on the user's next request, but an already-open stream stays open until it reconnects.
