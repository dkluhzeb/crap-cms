...
/// The dimensions of the upright image [`apply_exif_orientation`] produces
/// from `bytes`, given the `(width, height)` its header declares — known
/// without decoding a pixel. Orientations 5–8 turn the image a quarter, so
/// they swap the two.
pub(crate) fn upright_dimensions(bytes: &[u8], (width, height): (u32, u32)) -> (u32, u32) {
    match read_orientation(bytes) {
...
/// A minimal JPEG of `width`×`height` carrying an EXIF APP1 segment with the
/// given `Orientation`. Built by encoding a fresh JPEG and injecting the
/// segment by hand right after SOI.
#[cfg(test)]
#[allow(clippy::cast_possible_truncation)]
pub(crate) fn jpeg_with_orientation(width: u32, height: u32, orientation: u8) -> Vec<u8> {
    ...
    // APP1 segment layout (big-endian):
    //   FF E1                      — APP1 marker
    ...
    //   <12-byte IFD entry>        — Orientation tag (0x0112), SHORT, count 1
...
mod tests {
    ...
    /// Hand-built minimal JPEG (8x4) with an EXIF APP1 marker setting
    /// `Orientation = 6` (rotate 90° CW). Verifies that the EXIF reader picks
    /// up the tag and the helper rotates the decoded image accordingly.
    #[test]
    fn end_to_end_jpeg_with_orientation_6_is_rotated() {
        let with_exif = jpeg_with_orientation(8, 4, 6);
    ...
    /// The dimensions known from the header agree with the image the decode
    /// path produces: a quarter turn swaps them, a half turn or a mirror does
    /// not.
    #[test]
    fn upright_dimensions_agree_with_the_decoded_image() {
        for orientation in 1..=8 {
            let bytes = jpeg_with_orientation(8, 4, orientation);

            let decoded = image::load_from_memory(&bytes).expect("decode jpeg");
            let upright = apply_exif_orientation(&bytes, decoded);

            assert_eq!(
                upright_dimensions(&bytes, (8, 4)),
