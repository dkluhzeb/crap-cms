...
pub fn build_enriched_sub_field_context(
    ...
    match &sf.field_type {
        ...
        FieldType::Select | FieldType::Radio => field_types::sub_select_radio(&mut sub_ctx, sf, &val),
        ...
        FieldType::Array => field_types::sub_array(&mut sub_ctx, sf, raw_value, &indexed_name, locale_locked, non_default_locale, depth, errors),
        FieldType::Blocks => field_types::sub_blocks(&mut sub_ctx, sf, raw_value, &indexed_name, locale_locked, non_default_locale, depth, errors),
        FieldType::Group => field_types::sub_group(&mut sub_ctx, sf, raw_value, &indexed_name, locale_locked, non_default_locale, depth),
        FieldType::Row | FieldType::Collapsible => field_types::sub_row_collapsible(&mut sub_ctx, sf, raw_value, &indexed_name, locale_locked, non_default_locale, depth, errors),
        FieldType::Tabs => field_types::sub_tabs(&mut sub_ctx, sf, raw_value, &indexed_name, locale_locked, non_default_locale, depth, errors),
        FieldType::Text | FieldType::Number if sf.has_many => field_types::sub_has_many_tags(&mut sub_ctx, &val),
        _ => {}
    }
    ...
    for (ctx, field_def) in fields.iter_mut().zip(defs_iter) {
        match field_def.field_type {
            FieldType::Relationship => {
                field_types::enrich_relationship(ctx, field_def, doc_fields, &conn, reg, rel_locale_ctx.as_ref());
            }
            FieldType::Array => {
                field_types::enrich_array(ctx, field_def, doc_fields, state, non_default_locale, errors, &conn, reg, rel_locale_ctx.as_ref());
            }
            FieldType::Upload => {
                field_types::enrich_upload(ctx, field_def, doc_fields, &conn, reg, rel_locale_ctx.as_ref());
            }
            FieldType::Blocks => {
                field_types::enrich_blocks(ctx, field_def, doc_fields, state, non_default_locale, errors, &conn, reg, rel_locale_ctx.as_ref());
            }
            FieldType::Row | FieldType::Collapsible => {
            ...
            FieldType::Join => {
                field_types::enrich_join(ctx, field_def, &conn, reg, rel_locale_ctx.as_ref(), doc_id);
            }
            FieldType::Richtext => {
