//! Unit tests for the template registry and the compiled-in templates it
//! registers.
...
fn create_handlebars_loads_templates() {
    ...
    let hbs = create_handlebars(tmp.path(), false, translations, None).expect("create_handlebars");
...
fn base_layout_i18n_island_renders_valid_json() {
    let tmp = tempfile::tempdir().expect("tempdir");
    let translations = Arc::new(Translations::load(tmp.path()));
    let mut hbs = (*create_handlebars(tmp.path(), false, translations, None).expect("hbs")).clone();
...
/// Render `layout/<name>` around a fixed body with the given UI locale and
/// page-title key, the way the auth-flow and admin handlers drive them.
fn render_layout(layout: &str, locale: &str, title_key: &str) -> String {
    let tmp = tempfile::tempdir().expect("tempdir");
    let translations = Arc::new(Translations::load(tmp.path()));
    let mut hbs = (*create_handlebars(tmp.path(), false, translations, None).expect("hbs")).clone();

    let page = format!("{{{{#> layout/{layout}}}}}body{{{{/layout/{layout}}}}}");
    hbs.register_template_string("page", page)
        .expect("register page");
    ...
    hbs.render(
        ...
        &json!({
            "_locale": locale,
            "crap": { "csp_nonce": "n", "site_name": "Crap CMS" },
            "nav": { "collections": [], "globals": [] },
...
/// Regression: the auth-flow layout rendered the raw title key
/// (`login_page_title-Crap CMS`) and announced every page as English.
#[test]
fn auth_layout_translates_title_and_sets_ui_lang() {
    let html = render_layout("auth", "de", "login_page_title");

    assert!(
        html.contains("<title>Anmelden - Crap CMS</title>"),
        "auth title must be translated with a spaced separator: {html}"
    );
    assert!(
        html.contains(r#"<html lang="de">"#),
        "auth lang must follow the UI locale: {html}"
    );
}

/// Regression: the admin layout title lost the spaces around its separator.
#[test]
fn base_layout_title_has_spaced_separator() {
    let html = render_layout("base", "en", "login_page_title");

    assert!(
        html.contains("<title>Login - Crap CMS</title>"),
        "admin title must use ' - ' as separator: {html}"
    );
}
...
fn dev_mode_enables_dev_mode() {
    let tmp = tempfile::tempdir().expect("tempdir");
    let translations = Arc::new(Translations::load(tmp.path()));
    let hbs = create_handlebars(tmp.path(), true, translations, None).expect("create_handlebars");
    assert!(hbs.dev_mode());
}
