...
mod tests {
    ...
    fn and_strings() {
        ...
        hbs.register_template_string("t", "{{#if (and a b)}}YES{{else}}NO{{/if}}")
            .unwrap();
        assert_eq!(
            hbs.render("t", &serde_json::json!({"a": "hello", "b": "world"}))
                .unwrap(),
            "YES"
        );
        assert_eq!(
            hbs.render("t", &serde_json::json!({"a": "hello", "b": ""}))
                .unwrap(),
            "NO"
        );
    }
    ...
    fn and_numbers() {
        ...
        hbs.register_template_string("t", "{{#if (and a b)}}YES{{else}}NO{{/if}}")
            .unwrap();
        assert_eq!(
            hbs.render("t", &serde_json::json!({"a": 1, "b": 2}))
                .unwrap(),
            "YES"
        );
        assert_eq!(
            hbs.render("t", &serde_json::json!({"a": 1, "b": 0}))
                .unwrap(),
            "NO"
        );
    }
