...
pub fn run(
    ...
    let mut table = Table::new(vec!["Collection", "Rows", "Time"]);
    let mut explain_output: Vec<(String, Vec<String>)> = Vec::new();
        ...
        if explain {
            match run_explain(conn, slug, &find_query, def) {
                Ok(lines) if !lines.is_empty() => {
                    explain_output.push((slug.to_string(), lines));
                }
                Err(e) => {
    ...
    if !explain_output.is_empty() {
        println!();
        cli::header("Query Plans");

        for (slug, lines) in &explain_output {
            println!();
