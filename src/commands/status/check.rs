...
use crate::{
    ...
    core::{Hooks, LiveMode, Registry, collection::Auth},
    db::{DbConnection, DbPool},
};

use super::migrations::migration_status;
use collections::{check_auth_methods, check_collections};
...
fn check_migrations(pool: &DbPool, config_dir: &Path, findings: &mut Vec<Finding>) {
    let status = match migration_status(config_dir, pool) {
        Ok(status) => status,
        Err(e) => {
            findings.push(Finding::new(format!(
                "Could not read the migration status: {e:#}"
            )));
            return;
        }
    };
