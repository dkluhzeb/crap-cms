...
pub(super) fn render(registry: &Registry) -> String {
    ...
    w!(out);
    w!(out, "use serde::{{Deserialize, Serialize}};");
    w!(out);
...
fn render_rel_enum(out: &mut String) {
    ...
    w!(out, "/// or a populated document (depth>=1).");
    w!(out, "#[derive(Debug, Clone, Serialize, Deserialize)]");
    w!(out, "#[serde(untagged)]");
    w!(out, "pub enum Rel<T> {{");
    w!(out, "    Doc(Box<T>),");
    w!(out, "    Id(String),");
    w!(out, "}}");
    w!(out);
    w!(out, "impl<T> Rel<T> {{");
    w!(out, "    /// Get the ID if unpopulated.");
    w!(out, "    pub fn as_id(&self) -> Option<&str> {{");
    w!(
        out,
    ...
    w!(out, "    }}");
    w!(out);
    w!(out, "    /// Get the populated document.");
    w!(out, "    pub fn as_doc(&self) -> Option<&T> {{");
    w!(
        out,
...
fn render_collection(out: &mut String, col: &CollectionDefinition) {
    ...
    if col.timestamps {
        ...
        w!(out, "    pub created_at: Option<String>,");
        w!(
            out,
            "    #[serde(skip_serializing_if = \"Option::is_none\")]"
        );
        w!(out, "    pub updated_at: Option<String>,");
    }
    w!(out, "}}\n");
}
...
fn write_field_with_context(out: &mut String, field: &FieldDefinition, parent_pascal: &str) {
    ...
    if is_optional(field) {
        ...
        w!(out, "    pub {}: Option<{}>,", field.name, rust_type);
    } else {
        w!(out, "    pub {}: {},", field.name, rust_type);
    }
}
