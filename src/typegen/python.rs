...
/// Render all Python type definitions.
pub(super) fn render(registry: &Registry) -> String {
    ...
    w!(out, "from dataclasses import dataclass, field");
    w!(out, "from typing import Optional, Any\n");
...
fn render_collection(out: &mut String, col: &CollectionDefinition) {
    ...
    if col.timestamps {
        w!(out, "    created_at: Optional[str] = None");
        w!(out, "    updated_at: Optional[str] = None");
    }
    w!(out);
}
...
fn write_field_with_context(out: &mut String, field: &FieldDefinition, parent_pascal: &str) {
    ...
    if is_optional(field) {
        w!(out, "    {}: Optional[{}] = None", field.name, py_type);
    } else {
        let default = default_for_type(field);
        w!(out, "    {}: {} = {}", field.name, py_type, default);
    }
}
