...
impl ClientPrinter for PyPrinter {
    ...
    fn document(&mut self, def: &Document) {
        let base = idents::python_class(&def.name);
        let name = if def.localized {
            format!("{base}Localized")
        } else {
            base
        };
        let comment = if def.localized {
            "Read with locale=all: localized fields hold one value per locale.".to_string()
        } else {
...
/// Emit one read dataclass attribute — every field is optional on read: an
/// optional polymorphic-target comment, then `attr: Optional[type] = None`,
/// noting the wire key when the attribute was renamed.
...
fn py_ty(ty: &FieldTy) -> String {
    match ty {
        ...
        // A locale without a value reads as `None`.
        FieldTy::Localized(inner) => format!("dict[str, Optional[{}]]", py_ty(inner)),
...
mod tests {
    ...
    fn python_sanitizes_unsafe_names_and_list_defaults() {
        ...
        assert!(
            out.contains("class_: Optional[str]"),
            "keyword field → class_: {out}"
        );
        assert!(out.contains("# wire: class"), "renamed notes wire: {out}");
        assert!(out.contains("n2fa:"), "leading-digit field → n2fa: {out}");
        assert!(
            out.contains("tags: Optional[list[N2faTags]] = None"),
            "a list is optional on read: {out}"
        );
    }
    ...
    fn python_collection_output() {
        ...
        assert!(out.contains("    title: Optional[str] = None"));
    ...
    fn python_number_and_checkbox_fields() {
        ...
        let mut out = String::new();
        render_collection(&mut out, &col);
        assert!(out.contains("price: Optional[float] = None"));
    ...
    fn python_array_field_with_subfields() {
        ...
        assert!(out.contains("label: Optional[str] = None"));
    ...
    fn python_group_field_with_subfields() {
        ...
        assert!(out.contains("title: Optional[str]"), "sub-field: {out}");
        assert!(
    ...
    fn upload_has_many_generates_array_type() {
        ...
        render_collection(&mut out, &col);
        assert!(
            out.contains("images: Optional[list[str]]"),
            "has-many upload: {out}"
        );
    }
    ...
    fn python_select_with_options_docstring() {
        ...
        assert!(
            out.contains("status: Optional[Literal[\"draft\", \"published\"]] = None"),
            "select narrows to Literal: {out}"
        );
    }
    ...
    fn python_global_output() {
        ...
        assert!(out.contains("id: str = \"\""));
        assert!(out.contains("site_name: Optional[str] = None"));
    ...
    fn python_text_has_many() {
        ...
        render_collection(&mut out, &col);
        assert!(
            out.contains("tags: Optional[list[str]] = None"),
            "has-many text list: {out}"
        );
        assert!(
    ...
    fn python_number_has_many() {
        ...
        render_collection(&mut out, &col);
        assert!(
            out.contains("scores: Optional[list[float]] = None"),
            "has-many number list: {out}"
        );
        assert!(
    ...
    fn python_email_date_richtext_textarea() {
        ...
        let mut out = String::new();
        render_collection(&mut out, &col);
        assert!(out.contains("contact: Optional[str] = None"));
        assert!(out.contains("at: Optional[str] = None"));
        assert!(out.contains("body: Optional[str] = None"));
        assert!(out.contains("notes: Optional[str] = None"));
    }
    ...
    fn python_code_join_radio_fields() {
        ...
        render_collection(&mut out, &col);
        assert!(
            out.contains("snippet: Optional[str] = None"),
            "code → str: {out}"
        );
        assert!(
        ...
        assert!(
            out.contains("color: Optional[str] = None"),
    ...
    fn python_select_has_many() {
        ...
        render_collection(&mut out, &col);
        assert!(
            out.contains("tags: Optional[list[str]]"),
            "select has-many: {out}"
        );
        assert!(
    ...
    fn python_row_collapsible_tabs_promote_subfields() {
        ...
        assert!(
            out.contains("first_name: Optional[str] = None"),
            "row sub: {out}"
        );
        assert!(
        ...
        assert!(
            out.contains("tab_field: Optional[str] = None"),
            "tabs sub: {out}"
        );
    }
}
