...
use crate::{
    ...
    service::{
        ...
        build_hook_data, run_after_change_hooks,
    },
};
...
pub fn update_global_core(
    ...
    write_hooks: &dyn super::write_hooks::WriteHooks,
    slug: &str,
    def: &GlobalDefinition,
    input: WriteInput<'_>,
    user: Option<&Document>,
    ...
    let doc = if is_draft && def.has_versions() {
        ...
        versions::save_draft_version(
            conn, &gtable, "default", &def.fields,
            def.versions.as_ref(), &existing_doc, &final_ctx.data,
        )?;
        existing_doc
    } else {
        let locale_cfg = input.locale_ctx.map(|lctx| lctx.config.clone()).unwrap_or_default();
        let old_refs = query::ref_count::snapshot_outgoing_refs(conn, &gtable, "default", &def.fields, &locale_cfg)?;

        let doc = query::update_global(conn, slug, def, &final_data, input.locale_ctx)?;
        query::save_join_table_data(conn, &gtable, &def.fields, "default", &final_ctx.data, input.locale_ctx)?;
