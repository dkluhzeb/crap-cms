...
pub(super) fn collection_upload_to_lua(
    ...
    let Some(ref upload) = def.upload else {
        return Ok(());
    };
    ...
    // All defaults → shorthand `upload = true`
    let is_default = upload.mime_types.is_empty()
