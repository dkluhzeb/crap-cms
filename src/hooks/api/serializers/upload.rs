...
pub(super) fn collection_upload_to_lua(lua: &Lua, tbl: &Table, def: &crate::core::CollectionDefinition) -> mlua::Result<()> {
...
mod tests {
    ...
    fn test_collection_config_to_lua_with_upload() {
        ...
        def.timestamps = true;
        {
            ...
            upload.image_sizes = vec![
                ImageSizeBuilder::new("thumb").width(200).height(200).fit(crate::core::upload::ImageFit::Cover).build(),
            ];
    ...
    fn test_collection_config_to_lua_image_fit_variants() {
        ...
        for (fit, expected_str) in fits {
            ...
            upload.enabled = true;
            upload.image_sizes = vec![
                ImageSizeBuilder::new("thumb").width(100).height(100).fit(fit).build(),
            ];
            def.upload = Some(upload);
            let tbl = collection_config_to_lua(&lua, &def).unwrap();
            let upload: mlua::Table = tbl.get("upload").unwrap();
