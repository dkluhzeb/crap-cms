//! Top-level `CrapConfig` struct, loading, version check, and path helpers.
//! Per-section validation lives in `validate.rs`.
...
#[cfg(unix)]
pub(super) fn is_world_accessible_mode(mode: u32) -> bool {
...
#[cfg(all(unix, test))]
pub(super) fn should_warn_loose_permissions(config: &CrapConfig, mode: u32) -> bool {
